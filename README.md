...
Що таке CompletableFuture і для чого він використовується?
Відповідь: CompletableFuture — це клас у Java, який представляє майбутнє значення або результат асинхронної операції. Він дозволяє виконувати асинхронні обчислення, комбінувати їх, обробляти результати та обробляти помилки. CompletableFuture надає зручні методи для роботи з асинхронними задачами, такі як runAsync(), supplyAsync(), thenApplyAsync(), thenAcceptAsync() та thenRunAsync().

Яка різниця між методами runAsync() та supplyAsync()?
Відповідь: Метод runAsync() використовується для виконання асинхронної задачі, яка не повертає результат (тип Runnable). Метод supplyAsync(), навпаки, використовується для виконання асинхронної задачі, яка повертає результат (тип Supplier<T>).

Як працює метод thenApplyAsync() і коли його слід використовувати?
Відповідь: Метод thenApplyAsync() використовується для обробки результату асинхронної задачі та повернення нового результату. Він приймає функцію, яка застосовується до результату попередньої задачі, і повертає новий CompletableFuture з обробленим результатом. Цей метод слід використовувати, коли потрібно виконати додаткові обчислення на основі результату попередньої задачі.

Що робить метод thenCombineAsync() і як він використовується у вашій програмі?
Відповідь: Метод thenCombineAsync() використовується для комбінування результатів двох незалежних CompletableFuture. Він приймає два CompletableFuture та функцію, яка об'єднує їх результати в один. У нашій програмі цей метод використовується для обчислення факторіалу від суми елементів двох масивів.

Чому ми використовуємо BigInteger для обчислення факторіалу?
Відповідь: Ми використовуємо BigInteger для обчислення факторіалу, оскільки факторіал великих чисел може перевищувати діапазон типу long. BigInteger дозволяє працювати з дуже великими числами без переповнення.

Як ви вимірюєте час виконання асинхронних операцій у вашій програмі?
Відповідь: Час виконання асинхронних операцій вимірюється за допомогою методів System.currentTimeMillis(). Ми зберігаємо час початку операції перед її виконанням і час завершення після її виконання. Різниця між цими значеннями дає час, витрачений на виконання операції.

Що таке метод thenAcceptAsync() і як його використовувати?
Відповідь: Метод thenAcceptAsync() використовується для обробки результату асинхронної задачі без повернення нового результату. Він приймає споживача (тип Consumer<T>), який обробляє результат попередньої задачі.

**AsyncSequenceSum**
---
**Cheatsheet**

Чому ми використовуємо обгортки DataWrapper і TimeWrapper у другому варіанті програми?
Відповідь: Ми використовуємо обгортки DataWrapper і TimeWrapper для зберігання даних і часу виконання асинхронних операцій окремо. Це дозволяє нам точно вимірювати час виконання кожної операції та уникати побічних ефектів. Обгортка DataTimeWrapper об'єднує ці два об'єкти, що спрощує передачу даних і часу між асинхронними задачами.

Як ви обчислюєте загальний час виконання всіх асинхронних операцій у другому варіанті програми?
Відповідь: У другому варіанті програми ми обчислюємо загальний час виконання всіх асинхронних операцій, сумуючи тривалість кожної операції, збережену в об'єктах TimeWrapper. Це дозволяє отримати точний час виконання без врахування накладних витрат, пов'язаних з асинхронним виконанням.